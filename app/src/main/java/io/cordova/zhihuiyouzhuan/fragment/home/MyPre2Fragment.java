...
import io.cordova.zhihuiyouzhuan.activity.MyDataActivity;
import io.cordova.zhihuiyouzhuan.activity.MyToDoMsg2Activity;
import io.cordova.zhihuiyouzhuan.activity.TeacherDataActivity;
import io.cordova.zhihuiyouzhuan.bean.BackInfor;
import io.cordova.zhihuiyouzhuan.bean.CountBean;
import io.cordova.zhihuiyouzhuan.bean.OAMessageBean;
import io.cordova.zhihuiyouzhuan.bean.Obj;
import io.cordova.zhihuiyouzhuan.bean.Root;
...
import io.cordova.zhihuiyouzhuan.activity.MyCollectionActivity;
  ...
  //  @BindView(R.id.tv_app_setting1)
    ...
    public void initView(View view) {
        ...
        if (Integer.parseInt(count) > 99) {
            ...
            tvMyToDoMsgNum.setText(count); // 需要显示的提醒类容
    ...
    private void remind() { //BadgeView的具体使用
        ...
        count = (String) SPUtils.get(getContext(), "count", "");;
        if(count.equals("")){
            count = "0";
        }
        if (Integer.parseInt(count) > 99) {
...
//            tvDataNum.setText("11");
    ...
    private void dbDataList() {
        OkGo.<String>post(UrlRes.HOME_URL + UrlRes.Query_workFolwDbList2)
                .params("userId",(String) SPUtils.get(MyApp.getInstance(),"userId",""))
                .params("type","1")
                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(Response<String> response) {
                       ...
                       bean = JSON.parseObject(response.body(), Root.class);


                        if (bean.getSuccess()) {
                            Log.i("消息列表",response.body());
                            if(bean.getObj().getBackInfo().size() > 0){
                                setRvOAMsgList();
                            }else {
    ...
    public void onViewClicked(View view) {
        ...
        switch (view.getId()) {
            case R.id.rv_user_data:
                if (isLogin){
                    String role =  (String)SPUtils.get(getActivity(),"rolecodes","");
                    if(role.equals("student")){
                        intent = new Intent(MyApp.getInstance(), MyDataActivity.class);
                        startActivity(intent);
                    }else{
                        intent = new Intent(MyApp.getInstance(), TeacherDataActivity.class);
                        startActivity(intent);
                    }
//                    intent = new Intent(MyApp.getInstance(), MyDataActivity.class);
//                    startActivity(intent);
            ...
            case R.id.tv_app_msg:
                if (isLogin){
                    ...
                    intent = new Intent(MyApp.getInstance(), MyToDoMsg2Activity.class);
                    startActivity(intent);
                }
                break;
...
//        ?memberId=admin&pwd=d632eeeb1548643667060e18656e0112
    ...
    private void setRvOAMsgList() {
        ...
        myOaToDoList.setLayoutManager(new LinearLayoutManager(getContext()){
          ...
          oaAdapter = new CommonAdapter<BackInfor>(getContext(),R.layout.item_to_do_my_msg,bean.getObj().getBackInfo()) {
              @Override
              protected void convert(ViewHolder holder, BackInfor backInfor, int position) {
                  ...
                  holder.setText(R.id.tv_name,bean.getObj().getBackInfo().get(position).getRUN_NAME());
//                  holder.setText(R.id.tv_present,objBean.getTitle());
                  holder.setVisible(R.id.tv_present,false);
              }
        ...
        oaAdapter.setOnItemClickListener(new MultiItemTypeAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, RecyclerView.ViewHolder holder, int position) {
                if (!bean.getObj().getBackInfo().get(position).getLinkUrl().isEmpty()){
                    Log.e("url  ==",bean.getObj().getBackInfo().get(position).getLinkUrl()+ "");
                    if (null != bean.getObj().getBackInfo().get(position).getLinkUrl()){
                        Intent intent = new Intent(MyApp.getInstance(), BaseWebActivity4.class);
                        intent.putExtra("appUrl",bean.getObj().getBackInfo().get(position).getLinkUrl());
                        intent.putExtra("oaMsg","oaMsg");
                        intent.putExtra("appName",bean.getObj().getBackInfo().get(position).getFLOW_NAME());

...
public class YsAppAdapter3 extends CommonAdapter<YsAppBean.Obj> {
    Context mcontext;
    List<YsAppBean.Obj .Apps> appdatas;
    List<YsAppBean.Obj > mdatas;
    public YsAppAdapter3(Context context, int layoutId, List<YsAppBean.Obj> datas) {
    ...
    protected void convert(ViewHolder holder, YsAppBean.Obj apps, int position) {
        ...
        recyclerView.setLayoutManager(new GridLayoutManager(mContext,4));
            ...
            if(mdatas.get(position).getModulesCode().contains("bsdt_")){
                appdatas.clear();
